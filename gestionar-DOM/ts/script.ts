import { createElement, append } from './gestionarDOM/components/gestionarElements';
import { createTable, fillTable } from './gestionarDOM/components/gestionarTaules';
...
function placePattern(grid: Grid, coords: Array<[number, number]>) {
    for (const [r, c] of coords) {
        if (r >= 0 && r < grid.length && c >= 0 && c < grid[0].length) grid[r][c] = '<';
    }
}

function gridToTableData(grid: Grid) {
    return grid.map(row => row.map(cell => cell || ''));
}

function shiftGrid(grid: Grid, delta: number): Grid {
