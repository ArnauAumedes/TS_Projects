 ...
 */
import { createElement, append, setAttr } from './gestionarElements.js';
/**
 * Crea una taula amb opcions i dades inicials.
 * Retorna l'element `<table>` creat.
 */
export function createTable(options, data) {
    const table = createElement('table', { classes: options === null || options === void 0 ? void 0 : options.classes, attrs: options === null || options === void 0 ? void 0 : options.attrs, id: options === null || options === void 0 ? void 0 : options.id });
 ...
 */
export function fillTable(table, data) {
    ...
    for (const rowData of data) {
        ...
        for (const cellData of rowData) {
            const td = createElement('td', { text: String(cellData) });
            append(tr, td);
        }
        append(tbody, tr);
    }
 ...
 */
export function setCell(table, rowIndex, colIndex, value) {
 ...
 */
export function insertRow(table, pos, rowData) {
    var _a;
    const tbody = (_a = table.tBodies[0]) !== null && _a !== void 0 ? _a : table.appendChild(createElement('tbody'));
    const index = (pos === undefined) ? -1 : pos;
    const tr = tbody.insertRow(index);
    if (rowData && rowData.length) {
        for (const cellValue of rowData) {
            const td = createElement('td', { text: String(cellValue) });
            append(tr, td);
        }
    }
    return tr;
 ...
 */
export function deleteRow(table, pos) {
    if (pos < 0 || pos >= table.rows.length)
        return false;
    table.deleteRow(pos);
    return true;
}
 ...
 */
export function insertColumn(table, colIndex, values) {
...
export function moveColumn(table, fromIndex, toIndex) {
    ...
    for (const row of rows) {
        if (fromIndex < 0 || fromIndex >= row.cells.length)
            continue;
...
export function readTable(table) {
    const rows = Array.from(table.rows);
    return rows.map(r => Array.from(r.cells).map(c => { var _a; return (_a = c.textContent) !== null && _a !== void 0 ? _a : ''; }));
}
/**
...
export function fillRow(row, values) {
    for (let i = 0; i < values.length; i++) {
        if (i < row.cells.length) {
            ...
            const td = createElement('td', { text: String(values[i]) });
            append(row, td);
        }
    }
}
 ...
 */
export function setTableAttributes(table, attrs) {
