...
function fCreateUsers() {
    ...
    let numUsers = parseInt(prompt("Quantes finestres vols obrir?", String(2)) || "2", 10);
    ...
    userWindows.slice(0, numUsers).forEach((win, i) => {
        try {
            ...
            if (!win || win.closed) {
                const opened = window.open(`${urlBase}?uid=${userID}`, `LocalChatUser${userID}_${Date.now()}`, `left=${left},top=${top},width=${width},height=${height}`);
        ...
        userWindows.forEach((win, i) => {
            try {
                if (!win || win.closed) {
                    const uid = i + 1;
                    userWindows[i] =
                        window.open(`user.html?uid=${uid}`, `LocalChatUser_reopen_${uid}_${Date.now()}`, `left=${(i % cols) * width},top=${Math.floor(i / cols) * height},width=${width},height=${height}`) || null;
                }
            }
            catch (err) {
