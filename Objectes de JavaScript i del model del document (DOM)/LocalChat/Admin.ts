...
function fCreateUsers() {
  ...
  const w2 = window.open(
    `${urlBase}?uid=2`,
    `Loca lChatUser2_${Date.now()}`,
    `left=${width},top=${height},width=${width},height=${height}`
  );
  console.log("w2:", w2);
  ...
  userWindows.forEach((win, i) => {
    try {
      ...
      if (!win || win.closed) {
        ...
        userWindows[i] = window.open(
          `user.html?uid=${i + 1}`,
          `LocalChatUser_reopen_${i}_${Date.now()}`,
          `left=${Math.floor(screen.availWidth / 2)},top=${i * Math.floor(screen.availHeight / 2)},width=${Math.floor(screen.availWidth / 2)},height=${Math.floor(screen.availHeight / 2)}`
        );
      }
    } catch (err) {
