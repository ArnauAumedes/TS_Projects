...
export type ModeAnimacioBarres =
  | "primeres-interval"
  | "consecutiu"
  | "simultani-ritme"
  | "simultani-mateix-temps";
 ...
 */
export function crearGraficBarres(
  valors: number[],
  container: HTMLElement,
  mode: ModeAnimacioBarres = "simultani-mateix-temps",
  tempsDurada: number = 1000
) {
  // Configuració bàsica
  const ampleBarra = 40;
  const separacio = 20;
  ...
  const ampleTotal =
    valors.length * ampleBarra + (valors.length - 1) * separacio;
  ...
  valors.forEach((valor, i) => {
    ...
    text.setAttribute(
      "x",
      String(i * (ampleBarra + separacio) + ampleBarra / 2)
    );
  ...
  if (mode === "primeres-interval") {
    ...
    barres.forEach((rect, i) => {
      animarBarra(
        ...
        valors[i],
        maxAlcada,
        tempsDurada * (valors[i] / maxAlcada)
      );
    });
