...
export function mousePosition(
	...
	if (div) {
		div.addEventListener("mousemove", (e: MouseEvent) => {
			let x = 0, y = 0;
			switch (tipo) {
				case "offset": x = e.offsetX; y = e.offsetY; break;
				case "client": x = e.clientX; y = e.clientY; break;
				case "page": x = e.pageX; y = e.pageY; break;
				case "screen": x = e.screenX; y = e.screenY; break;
			}
			if (div) div.textContent = `Posició del ratolí (${tipo}): X=${x}, Y=${y}`;
		});
	}
}
	...
	if (div) {
		div.addEventListener(
			...
			(e: WheelEvent) => {
				...
				if (div) div.textContent = `Moviment de la rodeta: ${direction === "up" ? "amunt" : "avall"}`;
...
export function mouseButtons(
	...
	function updateButtons(e: MouseEvent) {
		...
		if (div) div.textContent = `Botó principal: ${left ? "PREMUT" : "no"} | Botó secundari: ${right ? "PREMUT" : "no"}`;
	...
	div.addEventListener("mouseleave", () => {
		if (div) div.textContent = "Botó principal: no | Botó secundari: no";
	});
 ...
 * @param callback Opcional, funció a cridar amb l'estat
 */
export function mouseButtonsWithKeys(
	div: HTMLElement | null,
	callback?: (
