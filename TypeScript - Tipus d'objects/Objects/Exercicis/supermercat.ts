...
const poma: Producte = { codi: 1, descripcio: "Poma", preu: 0.5, pes: 0.2 };
const llet: Producte = { codi: 2, descripcio: "Llet", preu: 1.2, volum: 1 };
const pera: Producte = { codi: 6, descripcio: "Pera", preu: 0.6, pes: 0.25 };
const iogurt: Producte = {
  codi: 7,
  descripcio: "Iogurt",
  preu: 0.9,
  volum: 1,
 ...
 */
export function mostrarProducte(producte: Producte) {
  const info = [
    `Codi: ${producte.codi}`,
  ...
  if (producte.pes) info.push(`Pes: ${producte.pes} g`);
  if (producte.volum) info.push(`Volum: ${producte.volum} l`);
  if (producte.dte) {
 ...
 */
export function mostrarTotsProductes(productes: Producte[]) {
 ...
 */
export function buscarProducteRepetit(codi: number) {
 ...
 */
export function errorHandler(producte: Producte): void {
 ...
 */
export function afegirProductes(nouProducte: Producte) {
 ...
 */
export function getObjectsByValue(array: Producte[], atribut: keyof Producte, valor: any): Producte[] {
    ...
    return array.filter(objecte => objecte[atribut as keyof Producte] === valor);
}
 ...
 * @param valor Valor a buscar
 */
export function buscarIMostrarPerValor(array: Producte[], atribut: keyof Producte, valor: any): void {
